...
t_info falsaposicao(double a,
   ...
   #ifdef INFO
   printf("Resolucao pelo Metodo da Posicao Falsa \n");
   printf("-------------------------------------------------\n");
   ...
   #ifdef INFO
   imprimelinhafalsaposicao(k, a, b, xk, xant, f);
   #endif // INFO


   while (1){

     if ( fabs(b - a) < epsilon || fabs(f(xk)) < epsilon  || k > itermax ) break;
     if (f(a) * f(xk) < 0){
        b = xk;
     }
     else{
...
int main(int argc, char* argv[])
{
    ...
    if( f(a) * f(b) < 0 ){
      ...
      cin >> max_iter;
      t_info info;
      info = falsaposicao(a, b, epsilon, max_iter, f);
      cout << "Resolucao pelo Metodo da Posicao Falsa" << endl;
      print_info(info);
      cout << endl << endl;
    }else{
