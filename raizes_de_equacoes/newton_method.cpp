...
t_info newton(double xant,
  ...
  #ifdef INFO
  printf("Resolucao pelo Metodo da Newton \n");
  printf("-------------------------------------------------\n");
  printf("%3s %8s %8s %8s\n", "k" , "xk", "f(xk)", "x_{k}-x_{k-1}");
  printf("-------------------------------------------------\n");
  #endif
  ...
  while (1){
     ...
     k = k + 1;
     #ifdef INFO
     imprimelinhanewton(k,xant,xk, f);
     #endif

     if( fabs(f(xk)) < epsilon || k > max_iter || fabs(xk - xant) < epsilon ) break;
     xant = xk;
  }
...
int main(int argc, char* argv[])
{
    ...
    if( f(a) * f(b) < 0 ){
      ...
      cin >> max_iter;
      t_info info;
      info = newton(a, epsilon, max_iter, f, _f);
      cout << "Resolucao pelo Metodo de Newton" << endl;
      print_info(info);
      cout << endl << endl;
    }else{
